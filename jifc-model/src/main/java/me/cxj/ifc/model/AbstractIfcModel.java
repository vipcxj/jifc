 ...
 */
public abstract class AbstractIfcModel implements IfcModel {

    private static final org.slf4j.Logger LOGGER = LoggerFactory.getLogger(AbstractIfcModel.class);
    protected ModelMetaData modelMetaData = null;
    private final Set<IfcModelChangeListener> changeListeners = new LinkedHashSet<>();

    // Object with oid
    private BiMap<Long, IdEObject> objects;

    // Objects without oid, usually embedded when serialized
    private final Set<IdEObject> unidentifiedObjects = new HashSet<>();
    ...
    public AbstractIfcModel(PackageMetaData packageMetaData, Map<Integer, Long> pidRoidMap, int size) {
    ...
    public AbstractIfcModel(PackageMetaData packageMetaData, Map<Integer, Long> pidRoidMap) {
    ...
    protected void buildIndex() {
        ...
        for (Long key : objects.keySet()) {
            ...
            if (value != null) {
                List<? extends IdEObject> list = indexPerClass.computeIfAbsent(value.eClass(), k -> new ArrayList<>());
    ...
    private void buildIndexWithSuperTypes(IdEObject eObject, EClass eClass) {
        if (!indexPerClassWithSubTypes.containsKey(eClass)) {
            indexPerClassWithSubTypes.put(eClass, new ArrayList<>());
        }
    ...
    public Map<EClass, Map<String, IdEObject>> createIndex(String featureName) {
        Map<EClass, Map<String, IdEObject>> index = new HashMap<>();
        if (objects.isEmpty()) {
            return index;
        }
        for (EClassifier classifier : objects.values().iterator().next().eClass().getEPackage().getEClassifiers()) {
            if (classifier instanceof EClass) {
                ...
                index.put((EClass) classifier, map);
            }
        }
        EClass ifcRootEclass = packageMetaData.getEClass("IfcRoot");
        EStructuralFeature feature = ifcRootEclass.getEStructuralFeature(featureName);
        for (Long key : objects.keySet()) {
            IdEObject value = objects.get(key);
            if (ifcRootEclass.isSuperTypeOf(value.eClass())) {
                Object featureValue = value.eGet(feature);
                if (featureValue != null) {
                    index.get(value.eClass()).put((String)featureValue, value);
                }
            }
        }
        return index;
    ...
    public void buildGuidIndex() {
        guidIndex = createIndex("GlobalId");
    }

    public void buildNameIndex() {
        nameIndex = createIndex("Name");
    }
    ...
    private void sortComplexList(final ObjectIDM objectIDM, final EClass originalQueryClass, EList<IdEObject> list, EStructuralFeature eStructuralFeature) {
        ...
        ECollections.sort(list, (o1, o2) -> {
            int i = 1;
            for (EStructuralFeature eStructuralFeature1 : type.getEAllStructuralFeatures()) {
                if (objectIDM.shouldFollowReference(originalQueryClass, type, eStructuralFeature1)) {
                    Object val1 = o1.eGet(eStructuralFeature1);
                    Object val2 = o2.eGet(eStructuralFeature1);
                    if (val1 != null && val2 != null) {
                        if (eStructuralFeature1.getEType() instanceof EClass) {
                            if (eStructuralFeature1.getEType().getEAnnotation("wrapped") != null) {
...
//					}
    ...
    @Override
    public long commit(String comment) throws PublicInterfaceNotFoundException {
        throw new UnsupportedOperationException();
    }
    ...
    @Override
    public void checkin(long poid, String comment) throws PublicInterfaceNotFoundException {
    }
    ...
    @Override
    public void query(ObjectNode query, boolean assumeCompletePreload) throws PublicInterfaceNotFoundException {
    }
