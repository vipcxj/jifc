...
public class IfcParserWriterUtils {
			...
			if (instanceClass == Integer.class || instanceClass == int.class) {
				...
				if (value.startsWith("\"") && value.endsWith("\"")) {
					try {
                        ...
                        return Hex.decodeHex(substring.toCharArray());
	 ...
	 * @throws DeserializeException any exception
	 */
		...
		if (val.getClass().getSimpleName().equals("Tristate")) {
			...
			for (int i=0; i<stringVal.length(); i++) {
				...
				if (c == '\'') {
					...
					outputStream.write(("\\X\\" + new String(Hex.encodeHex(Charsets.ISO_8859_1.encode(CharBuffer.wrap(new char[]{c})).array())).toUpperCase()).getBytes(Charsets.UTF_8));
				} else {
					if (USE_ISO_8859_1) {
			...
			outputStream.write(val.toString().getBytes(Charsets.UTF_8));
		}
	}

	public static void writePrimitive(Object val, PrintWriter printWriter) throws SerializerException {
		if (val.getClass().getSimpleName().equals("Tristate")) {
            switch (val.toString()) {
                case "TRUE":
                    printWriter.write(".T.");
			...
			for (int i=0; i<stringVal.length(); i++) {
				...
				if (c == '\'') {
					...
					//  ISO 10646 and ISO 8859-1 are the same < 255 , using ISO_8859_1
					printWriter.write("\\X\\");
					printWriter.write(Hex.encodeHex(Charsets.ISO_8859_1.encode(CharBuffer.wrap(new char[]{c})).array(), false));
				} else {
					if (USE_ISO_8859_1) {
						// ISO 8859-1 with -128 offset
						printWriter.write("\\S\\");
